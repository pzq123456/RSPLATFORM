...
class Post(db.Model): # 用户发帖表
    ...
    timestamp = db.Column(db.DateTime, index=True, default=datetime.utcnow) # 更改为遥感影像上传的时间戳
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id')) # 外键与存入该遥感影像的用户相关联


    ####### 为遥感解译平台定制的字段：
    img_path=db.Column(db.String(300)) # 存储图片在服务器上的地址
    body = db.Column(db.String()) # 记录图片描述
...
class Follow_Post(db.Model): # 用户跟帖表
    ...
    img_path=db.Column(db.String(300)) # 存储图片在服务器上的地址 这里记录的已经处理好的结果图片 存储在resultimg文件夹中
